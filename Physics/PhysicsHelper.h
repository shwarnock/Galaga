...
inline bool BoxBoxCollision(BoxCollider* b1, BoxCollider* b2)
{
	...
	projectionAxis[0] = (b1->GetVertexPos(0) - b1->GetVertexPos(1)).Normalized();
	projectionAxis[1] = (b1->GetVertexPos(0) - b1->GetVertexPos(2)).Normalized();
	projectionAxis[2] = (b2->GetVertexPos(0) - b2->GetVertexPos(1)).Normalized();
	projectionAxis[3] = (b2->GetVertexPos(0) - b2->GetVertexPos(2)).Normalized();
	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
		{
			proj1 = Dot(b1->GetVertexPos(j), projectionAxis[i]);
			proj2 = Dot(b2->GetVertexPos(j), projectionAxis[i]);
		...
		float midPoint1 = b1Min;

		float halfDis2 = (b2Max - b2Min) * 0.5f;
		float midPoint2 = b2Min;
